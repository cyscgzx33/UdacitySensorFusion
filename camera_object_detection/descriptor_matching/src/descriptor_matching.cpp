...
{
    // configure matcher
    bool crossCheck = false; // Check point #1: crosCheck option turned ON / OFF
    ...
    else if (matcherType.compare("MAT_FLANN") == 0)
    {
        ...
        // TODO (done): implement FLANN matching
    ...
    { // k nearest neighbors (k=2)
        ...
        double t = (double)cv::getTickCount();
        // TODO (done): implement k-nearest-neighbor matching
        int normType = descriptorType.compare("DES_BINARY") == 0 ? cv::NORM_HAMMING : cv::NORM_L2;
        matcher->knnMatch(descSource, descRef, knn_matches, k);
        t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
        cout << " (KNN) with n=" << knn_matches.size() << " matches in " << 1000 * t / 1.0 << " ms" << endl;
        // TODO (done): filter matches using descriptor distance ratio test
...
int main()
{
    ...
    string descriptorType = "DES_GRADIENT";  // Note: If using SIFT dataset,set to "DES_GRADIENT" as SIFT is a gradient-based method instead of binary
                                             //       then, in the matcher initialized step, it would set to L2 norm instead of Hamming norm

...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SeparateClouds(pcl::PointIndices::Ptr inliers, typename pcl::PointCloud<PointT>::Ptr cloud) 
{
    ...
    // Create container for obstacles and plane: use the PointT template
    // Note: must claim "typename" here, as PointT is dependent with template
    typename pcl::PointCloud<PointT>::Ptr obstacle_cloud(new pcl::PointCloud<PointT>());
    typename pcl::PointCloud<PointT>::Ptr plane_cloud(new pcl::PointCloud<PointT>());

    // Grab the inliers' points as the plane cloud
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    pcl::SACSegmentation<PointT> seg; // need to use the template PointT
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    for (pcl::PointIndices indices : cluster_indices)
    {
        ...
        // because the output of the ec.extract() is bunch of indices
        // now we use the indices to access data within cloud->points
        for (int index : indices.indices)
            cloud_cluster->points.push_back(cloud->points[index]);

        cloud_cluster->width = cloud_cluster->points.size(); // why equal to size() ?
        cloud_cluster->height = 1;
        cloud_cluster->is_dense = true;

        // store in clusters container

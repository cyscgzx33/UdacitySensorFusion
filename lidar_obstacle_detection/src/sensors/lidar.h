...
struct Lidar
{
		...
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		...
		minDistance = 5.0;  // initial: 0.0
		maxDistance = 50;
		resoultion = 0.2;
		// TODO (done): set sderr to 0.2 to get more interesting pcd files
		sderr = 0.2;        // initial: 0.0, now added white noise
		cars = setCars;
		groundSlope = setGroundSlope;

		// TODO (done): increase number of layers to 8 to get higher resoultion pcd
		int numLayers = 8;  // initial: 3
		...
		double horizontalAngleInc = pi / 64;  // initial: pi / 6
